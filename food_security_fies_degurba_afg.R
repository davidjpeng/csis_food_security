...
# Read-in FAO FIES data. Public requests for access should be made here: https://microdata.fao.org/index.php/catalog
# We have included this data with permission for editor/reviewer peer review only. 

fies_data <- read_dta('fies_afghanistan/AFG_2022_FIES_v01_EN_M_v01_A_OCS_scrubbed.dta')

# NOTE: the data used in our research is augmented with GPS coordinates at the ultimate cluster level of the survey. These coordinates underwent differential privacy
# techniques to protect respondents while still enabling us to approximate reconstructing DEGURBA from the bottom up. We were able to access this through collaboration with the FAO and Gallup World Poll. 
# Public requests to these coordinates should be made to Gallup World Poll. 

gps_augmentation <- read_xlsx('gallup_provided_gps/Afghanistan_FIES_2022_coordinates.xlsx')
...
download.file(
  "https://geodata.ucdavis.edu/gadm/gadm4.1/gpkg/gadm41_AFG.gpkg",
  "gadm41_AFG.gpkg"
)

# Check the layers available 
st_layers('gadm41_AFG.gpkg')

# Convert to Mollweide, and select only relevant attributes
units_country <-
  sf::st_read('gadm41_AFG.gpkg', layer='ADM_ADM_2') %>% #layer is administrative level 2, which matches what GHS_DUC_Globe uses: https://human-settlement.emergency.copernicus.eu/ghs_duc2023.php
...
##QC
table(is.na(interviews_with_units$GID_2)) #are there any unassigned interviews?
...
#from Worldpop: https://hub.worldpop.org/geodata/listing?id=139
...
# Thanks to Luca Maffenini at the European Commission perfoming a fix to the tool, we are able to leverage GHS-POPWARP https://human-settlement.emergency.copernicus.eu/tools/GHS-POPWARP_User_Guide.pdf?rnd=0.9094042696948472 to perform the projection 
# to the Worldpop rasters. This conversion is done outside R and requires a Windows machine with POPWARP installed. You may test it yourself, where each .tif needs to be manually reprojected in the GUI.   

##0-14 

under_15_file_list <- list.files("worldpop/warped/afg/under_15", pattern=".tif", full.names = TRUE)
...
over_15_file_list <- list.files("worldpop/warped/afg/15+", pattern=".tif", full.names = TRUE)
...
as_tibble(under_15_mosaic) %>% summarise(total_0to14 = sum(afg_f_0_2020_constrained_UNadj_WARPED, na.rm=T))

as_tibble(over_15_mosaic) %>% summarise(total_15_plus = sum(afg_f_15_2020_constrained_UNadj_WARPED, na.rm=T))

as_tibble(combined) %>% summarise(total = sum(afg_f_0_2020_constrained_UNadj_WARPED, na.rm=T))

####Check if the sum of each age group is equal to the total: 
(as_tibble(under_15_mosaic) %>% summarise(total_0to14 = sum(afg_f_0_2020_constrained_UNadj_WARPED, na.rm=T)) ) + (as_tibble(over_15_mosaic) %>% summarise(total_15_plus = sum(afg_f_15_2020_constrained_UNadj_WARPED, na.rm=T)) ) == as_tibble(combined) %>% summarise(total = sum(afg_f_0_2020_constrained_UNadj_WARPED, na.rm=T))
...
# download the GHSL data on a global scale. Since our packaging of this repo, GHSL has changed some links on their website and this function would re-direct you to manually download. Therefore we provide code we used to download the data through R, but comment it out.

# dir.create('data-global')
# 
...
built_grid <- terra::crop(rast('data-global/BUILT_S.tif'), 
                          terra::ext(under_15_mosaic), 
                          filename='data-afg/BUILT_S.tif', overwrite=TRUE)

#land grid
land_grid <- terra::crop(rast('data-global/LAND.tif'), 
                         terra::ext(under_15_mosaic), 
                         filename='data-afg/LAND.tif', overwrite=TRUE)
...
writeRaster(under_15_mosaic ,'data-afg/POP_under_15.tif', overwrite=TRUE)
writeRaster(over_15_mosaic ,'data-afg/POP_15_over.tif', overwrite=TRUE)
writeRaster(combined ,'data-afg/POP_gen.tif', overwrite=TRUE)
...
units_country <- units_country %>%
  rename(UID = GID_2) #UID convention for Flexurba 

### Preprocess data 
data1 <- preprocess_units(
  units = units_country,
  classification = un_degurba, #object from Level 1, Stage 1
  pop = "data-afg/POP_gen.tif" #total population mosaic 
)
...
country_degurba_labels <- interviews_with_units %>%
  select(Random_ID, GID_2) %>%
  left_join(
    units_classification_country %>%
      select(UID, flexurba_L1), by=c("GID_2" = "UID")
  )

fies_data <- fies_data %>%
  left_join(as_tibble(country_degurba_labels) %>%
              select(Random_ID, GID_2, flexurba_L1) %>%
              rename(DEGURBA = flexurba_L1), by = "Random_ID"
  ) 

fies_data <- fies_data %>%
  select(-GID_2.x, -DEGURBA.x) %>% #Dropping the GWP variables 
  rename(GID.2 = GID_2.y) %>%
...
moe=function(prob,rs,wt,sd=NULL,psu=NULL,strata=NULL,conf.level=0.9){
  ...
  # return(list(moe=moe, se_m=se_m,se_s=se_s)) modified the return for ease of use in our analysis 
  return(moe[[1]])
}
...
ggplot(prevalence_summary_long %>% 
  ...
  labs(
    x = "DEGURBA",
    y = "Prevalence",
    title = "Afghanistan (AFG) - 2022 FIES Prevalence Estimates by DEGURBA, Severity, and Age Group",
    color = "Severity"
  ) +
  theme_minimal() +
...
outermost_boundary$GID_2 <- "country_boundary"  # Assign a unique identifier for the entire country boundary
...
buffered_country_raster <- terra::rasterize(buffered_country, under_15_mosaic, field = "GID_2", background = NA)
...
degurba_by_agegrp_buffered <- units_classification_country %>%
  ...
  left_join(
    zonal_stats_under15 %>%
      rename(under_15_population = afg_f_0_2020_constrained_UNadj_WARPED), by="UID" #note the name is carried over from the Worldpop files. Update for other countries. 
  ) %>%
  left_join(
    zonal_stats_over15 %>%
      rename(fifteen_plus_population = afg_f_15_2020_constrained_UNadj_WARPED), by="UID" #note the name is carried over from the Worldpop files. Update for other countries. 
  ) %>%
  group_by(flexurba_L1) %>%
...
### People estimates
write_csv(people_estimates_totalpopulation, "results/afg_people_estimates_totalpopulation.csv")
write_csv(people_estimates_by_agegrp, "results/afg_people_estimates_by_agegrp.csv")

### Population x DEGURBA 
write_csv(degurba_by_agegrp_buffered, "results/afg_degurba_population_by_agegrp.csv")

### DEGURBA x Small spatial units 
write_csv(units_classification_country, "results/afg_small_spatial_classified_DEGURBA.csv")